clear;close;clc
addpath("M:\FYS-STK4155\Project3\segymat-master\segymat-master")
addpath("M:\FYS-STK4155\Project3\1997_2.5D_shots.segy")

[Shots,SegyTraceHeaders,SegyHeader]=ReadSegy('1997_2.5D_shots.segy');

[A,B] = size(Shots);

start = 1;
stopp = 50;

imagesc(Shots(1:250,256*(start):256*(stopp-1)))
colormap(gray)
caxis([-0.1 0.1])

%% ============================= Make training data set ==========================
% Random draw individual shots from the data

nr_shots = 200;

nr = randi([1 B/256],1,nr_shots);


Shots_training = zeros(nr_shots,A,256);

for j = 1:nr_shots

    Shots_training(j,:,:) = Shots(:,( 256*(nr(j) - 1) + 1 ) : 256*nr(j) );
    
end


% Apply gain

[K,M,N] = size(Shots_training);

t2 = ([0:(M-1)]*0.004).^1.1;
%figure;plot(t2)
Shots_training_g = zeros(K,M,N);

for k = 1:K
    for i = 1:M

        Shots_training_g(k,i,:) = Shots_training(k,i,:)*t2(i);
        
    end
end


% Normalize the data

Shots_training_gn = zeros(K,350,200);

for k = 1:K
    Shots_training_gn(k,:,:) = Shots_training_g(k,1:350,1:200)/max(max(abs(Shots_training_g(k,1:350,1:200))));
end

figure(2);

for i = 1:20
    subplot(2,10,i)
    sh     = squeeze(Shots_training_gn(i,:,:));
    imagesc(sh)
    colormap(gray)
    caxis([-0.1 0.1])
end


save('Training_data_large.mat','Shots_training_gn');
