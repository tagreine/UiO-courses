...
def create_CNN_int(X, shape,ratio,samp_hr,samp_lr):
    with tf.name_scope('CNN_int'):
        ...
        filter_shape1 = [11, 11, 1, 32]
        filter_shape2 = [3, 3, 32, 32]
        filter_shape3 = [3, 3, 32, ratio]
        filter_shape4 = [5, 5, 1, 32]
        filter_shape5 = [3, 3, 32, 1]
        ...
        # Define weights and biases for the model. Fixed filter shapes        
        bias1 = bias_variables(filter_shape=32, name='b1')
        bias2 = bias_variables(filter_shape=32, name='b2')
        bias3 = bias_variables(filter_shape=1,  name='b3')
        bias4 = bias_variables(filter_shape=64, name='b4')
        bias5 = bias_variables(filter_shape=1,  name='b5')
        ...
        subpx = tf.contrib.periodic_resample.periodic_resample(a4, [shape[0],samp_hr, samp_hr, None])
        conv4 = tf.nn.conv2d(subpx, weight4, strides=[1,1,1,1], padding='SAME') + bias4
        a5    = tf.nn.leaky_relu(conv4,alpha=0.5)
        
        pred = tf.nn.conv2d(a5, weight5, strides=[1,1,1,1], padding='SAME') + bias5
