...
def create_CNN_int(X, shape,ratio,samp_hry, samp_hrx,samp_lr):
    with tf.name_scope('CNN_int'):
        ...
        #weight1 = tf.get_variable('weight1', shape=filter_shape1, initializer=tf.contrib.layers.variance_scaling_initializer())
        #weight2 = tf.get_variable('weight2', shape=filter_shape2, initializer=tf.contrib.layers.variance_scaling_initializer())
        #weight3 = tf.get_variable('weight3', shape=filter_shape3, initializer=tf.contrib.layers.variance_scaling_initializer())
        #weight4 = tf.get_variable('weight4', shape=filter_shape4, initializer=tf.contrib.layers.variance_scaling_initializer())
        #weight5 = tf.get_variable('weight5', shape=filter_shape5, initializer=tf.contrib.layers.variance_scaling_initializer())
        #weight6 = tf.get_variable('weight6', shape=filter_shape6, initializer=tf.contrib.layers.variance_scaling_initializer())

        
        # Define weights and biases for the model. Fixed filter shapes        
        bias1 = bias_variables(filter_shape=64,    name='b1')
        bias2 = bias_variables(filter_shape=ratio, name='b2')
        ...
        #a3    = tf.nn.tanh(conv2)        
        conv3 = tf.nn.conv2d_transpose(a3, weight3, output_shape=(shape[0],samp_hry,samp_hrx,1), strides=[1,1,ratio,1], padding='SAME') + bias3
        #a4    = tf.nn.elu(conv3)
        a4    = tf.nn.leaky_relu(conv3,alpha=0.5)
        #a4    = tf.nn.tanh(conv3)
        conv4 = tf.nn.conv2d(a4, weight4, strides=[1,1,1,1], padding='SAME') + bias4
        #a5    = tf.nn.elu(conv4)
        a5    = tf.nn.leaky_relu(conv4,alpha=0.5)
        #a5    = tf.nn.tanh(conv4)
        conv5 = tf.nn.conv2d(a5, weight5, strides=[1,1,1,1], padding='SAME') + bias5
        #a6    = tf.nn.elu(conv5)
        a6    = tf.nn.leaky_relu(conv5,alpha=0.5)
        #a6    = tf.nn.tanh(conv5)
        
        pred  = tf.nn.conv2d(a6, weight6, strides=[1,1,1,1], padding='SAME') + bias6
...
def Interpolation_model(x, shape, ratio, params, samp_hry, samp_hrx, samp_lr):
        ...
        a3    = tf.nn.leaky_relu(conv2,alpha=0.5)
        #a3    = tf.nn.tanh(conv2)        
        conv3 = tf.nn.conv2d_transpose(a3, params['W3'], output_shape=(shape[0],samp_hry,samp_hrx,1), strides=[1,1,ratio,1], padding='SAME') + params['B3']
        #a4    = tf.nn.elu(conv3)
        a4    = tf.nn.leaky_relu(conv3,alpha=0.5)
        #a4    = tf.nn.tanh(conv3)
        conv4 = tf.nn.conv2d(a4, params['W4'], strides=[1,1,1,1], padding='SAME') + params['B4']
        #a5    = tf.nn.elu(conv4)
        a5    = tf.nn.leaky_relu(conv4,alpha=0.5)
        #a5    = tf.nn.tanh(conv4)
        conv5 = tf.nn.conv2d(a5, params['W5'], strides=[1,1,1,1], padding='SAME') + params['B5']
        #a6    = tf.nn.elu(conv5)
        a6    = tf.nn.leaky_relu(conv5,alpha=0.5)
        #a6    = tf.nn.tanh(conv5)
        
        pred  = tf.nn.conv2d(a6, params['W6'], strides=[1,1,1,1], padding='SAME') + params['B6']
    
        return pred
    
#======================================================================================================        
# Define tensorflow optimization    
