...
def create_placeholders(X_train,X_test,samp_hr,samp_lr):
    with tf.name_scope('data'):
        ...
        xt = tf.placeholder(tf.float32, [shapet[0], samp_hr, samp_lr, 1], name='test_inputs')
        y  = tf.placeholder(tf.float32, [shape[0],  samp_hr, samp_hr, 1], name='training_targets')
        yt = tf.placeholder(tf.float32, [shapet[0], samp_hr, samp_hr, 1], name='test_targets')
...
def create_CNN_int(X, shape,ratio,samp_hr,samp_lr):
    with tf.name_scope('CNN_int'):
        ...
        #a3    = tf.nn.tanh(conv2)        
        conv3 = tf.nn.conv2d_transpose(a3, weight3, output_shape=(shape[0],samp_hr,samp_hr,1), strides=[1,1,ratio,1], padding='SAME') + bias3
        #a4    = tf.nn.elu(conv3)
...
def Interpolation_model(x, shape, ratio, params, samp_hr, samp_lr):
        ...
        a3    = tf.nn.leaky_relu(conv2,alpha=0.5)        
        conv3 = tf.nn.conv2d_transpose(a3, params['W3'], output_shape=(shape[0],samp_hr,samp_hr,1), strides=[1,1,ratio,1], padding='SAME') + params['B3']
        a4    = tf.nn.leaky_relu(conv3,alpha=0.5)
