...
axarr[0].imshow(X_train,**cmap_args)
axarr[0].set_title('Training Design Matrix',fontsize=16)
axarr[0].tick_params(labelsize=16)

im=axarr[1].imshow(X_test,**cmap_args)
axarr[1].set_title('Test Design Matrix',fontsize=16)
axarr[1].tick_params(labelsize=16)


fig.subplots_adjust(right=1.0)
plt.savefig('Results_Regression/Training_test_matrix.png', dpi=600,bbox_inches = 'tight')
...
# PLotting best fit for lasso on test set

Y_pred_train_lasso, beta_lasso = least_square(X_train,Y_train, method='Lasso',lamb=0.001,intercept='False') # train model, and store weigths 
Y_pred_test_lasso  = X_test.dot(beta_lasso) # fit model 
Y_pred_test_lasso  = Y_pred_test_lasso.reshape(Y_test.shape[0],1)

xrange = np.arange(200)

x1t = xrange[0:50].reshape(50,1)
y1t = Y_test[0:50].reshape(50,1)
y1tp= Y_pred_test_lasso[0:50].reshape(50,1)

x2t = xrange[100:150].reshape(50,1)
y2t = Y_test[100:150].reshape(50,1)
y2tp = Y_pred_test_lasso[100:150].reshape(50,1)

fig, axarr = plt.subplots(nrows=1, ncols=3)

axarr[0].plot(xrange,Y_test, '*--r',xrange,Y_pred_test_lasso, '*--k', lw=1)
axarr[0].legend(('Truth','Predicted test'))
axarr[0].set_ylabel('$\\mathrm{E[s]}$')
#axarr[0].set_xlabel('$\\$')
axarr[0].grid()

axarr[1].plot(x1t,y1t, '*--r',x1t,y1tp, 'k',lw=1)
axarr[1].legend(('Truth','Predicted test'))
axarr[1].set_ylabel('$\\mathrm{E[s]}$')
#axarr[1].set_xlabel('$\\$')
axarr[1].grid()

axarr[2].plot(x2t,y2t, '*--r',x2t,y2tp, 'k',lw=1)
axarr[2].legend(('Truth','Predicted test'))
...
plt.savefig('Results_Regression/Train_fit_lasso_001.png', dpi=600,bbox_inches = 'tight')
