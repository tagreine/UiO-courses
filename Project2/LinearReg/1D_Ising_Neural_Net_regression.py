...
train_err_nn   = np.zeros([len(lmbdas),1])
test_err_nn    = np.zeros([len(lmbdas),1])
train_r2_nn    = np.zeros([len(lmbdas),1])
test_r2_nn     = np.zeros([len(lmbdas),1])
...
X   = X_train[0:2000,:].reshape(2000,n).T
Y   = Y_train[0:2000,:].reshape(2000,1).T
X_t = X_test[0:2000,:].reshape(2000,n).T
Y_t = Y_test[0:2000,:].reshape(2000,1).T
...
for lm in lmbdas:
    ...
    #===========================================================================================================================================================================================================
    cost_SGD, W1, W2, W3, bias1, bias2, bias3 = neural_net_batch( X, Y, hiddenLayerSize1,hiddenLayerSize2, epochs = 100, batch_size = 20, alpha = 0.01, lmbda = lm, activation= 'sigmoid', intercept = 'False')
    ...
    train_err_nn[i] = MSE_metric(Y_train, Y_pred_train_ols)
    test_err_nn[i]  = MSE_metric(Y_test, Y_pred_test_ols)
    
    train_r2_nn[i] = R2_metric(Y_train, Y_pred_train_ols)
    test_r2_nn[i]  = R2_metric(Y_test, Y_pred_test_ols)
